...
In the case of something bad happening with the compiled version of a gem, you can revert to the non-compiled version of the gem using `macgem decompile`.

== TODO

* A gem install option would be most natural
  * gem install --compile nokogiri
* Compile dependency gems that get installed
* Timestamp detection to avoid recompiling files
* Code parsing to WARN about gems that will not work when compiled
  * This could possibly allow for smaller/combined binary files
